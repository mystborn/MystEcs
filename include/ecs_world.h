...
// count: A pointer filled with the number of entities on the world.
// returns an array of ComponentEnum where each index corresponds to an Entity.
ComponentEnum* ecs_world_get_components(EcsWorld world, int* count);

// A flag that determines if an entity is alive.
extern ComponentFlag is_alive_flag;

// A flag that determines if an entity is enabled.
extern ComponentFlag is_enabled_flag;
